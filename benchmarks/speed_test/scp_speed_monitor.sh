...
DEST_USER="uab586750"
DEST_HOST="transfer1.bsc.es"
DEST_DIR="/gpfs/home/uab/uab586750"

INTERVAL_SECONDS=$((30*60))   # 2 hours
LOG_DIR="$HOME/scp_speed_logs"
...
command -v scp >/dev/null 2>&1 || { echo "scp not found"; exit 127; }
...
while true; do
  ...
  set +e
  # Show SCP progress (do NOT use -q). time output goes to tmp_time so progress remains visible.
  sshpass -p "$SSH_PASSWORD" \
    /usr/bin/time -f "REAL_SECONDS=%e" -o "$tmp_time" \
    scp -o Compression=no "${SSH_OPTS[@]}" \
      "$SRC" "${DEST_USER}@${DEST_HOST}:${remote_path}"
  rc=$?
  set -e
