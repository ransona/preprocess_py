...
def bv_timing_bug(userID, expID,plot_on,filter_on=False):
    ...
    try:
        ...
        try:
            harp_data_path = os.path.join(exp_dir_raw, expID + '_Behavior_44.bin')
            data_read = harp.io.read(harp_data_path)  
            data_read_np = np.array(data_read)
            data_read_np[0:400,0] = data_read_np[400,0]
            harp_pd = data_read_np[:,0]
            harp_encoder = data_read_np[:,1]
            harp_encoder = np.cumsum(harp_encoder)
            harp_pd_time = np.arange(0, len(harp_pd)/1000, 1/1000)            
        except:
            print('No Harp data found')
            return
    ...
    # same for max
    max_diff_dig_tl = np.max(np.diff(flip_times_dig_tl))
    print(f'Max diff between TL digital timing pulses: {max_diff_dig_tl}')
    # same for median
    median_diff_dig_tl = np.median(np.diff(flip_times_dig_tl))
    print(f'Median diff between TL digital timing pulses: {median_diff_dig_tl}')
    # show histogram of pulse widths
    # plt.figure()
    # plt.hist(np.diff(flip_times_dig_tl),bins=np.arange(0,2,0.01))
    # plt.title('TL digital pulse widths')
    ...
    min_pulses = min(len(flip_times_bv),len(flip_times_pd_tl),len(flip_times_harp))
    median_diff_pd_dig_tl = np.abs(np.median(flip_times_dig_tl[0:min_pulses] - flip_times_pd_tl[0:min_pulses]))
    max_diff_pd_dig_tl = np.max(np.abs(flip_times_dig_tl[0:min_pulses] - flip_times_pd_tl[0:min_pulses]))
    ...
    # Plotting  
    plt.figure()
    plt.plot(bv_timestamp_2_tl,Encoder/max(Encoder),label='BV')
    plt.plot(harp_timestamp_tl,harp_encoder/max(harp_encoder),label='Harp')
    plt.legend(['BV', 'Harp'])
    plt.show()    
